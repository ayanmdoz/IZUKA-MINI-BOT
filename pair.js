...
const config = {
    ...
    RCD_IMAGE_PATH: 'https://files.catbox.moe/a51qw5.jpeg',
    NEWSLETTER_JID: '120363401819417685@newsletter',
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    version: '1.0.0',
    OWNER_NUMBER: '258833406646',
    BOT_FOOTER: '> CRIADO POR AYAN MODZ',
...
const owner = 'AYANMDOZ';
const repo = 'IZUKA-MINI-BOT';
...
function getSriLankaTimestamp() {
    return moment().tz('Africa/Maputo').format('YYYY-MM-DD HH:mm:ss');
}
      ...
      buttons: [
        {
          ...
          buttonText: { displayText: 'IZUKA MINI BOT' },
          type: 4,
          nativeFlowInfo: {
            name: 'single_select',
            paramsJson: JSON.stringify({
              title: 'IZUKA MINI BOT COMMANDS',
              sections: [
                {
                  title: "🌐 ɢᴇɴᴇʀᴀʟ ᴄᴏᴍᴍᴀɴᴅs",
                  highlight_label: 'IZUKA MINI BOT',
                  rows: [
...
case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363401819417685@newsletter'
                        });
                    }
    ...
    break;
                ...
                await socket.sendMessage(sender, { react: { text: '📲', key: msg.key } });
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: `> *IZUKA MINI BOT ᴘᴀɪʀ ᴄᴏᴍᴘʟᴇᴛᴇᴅ* ✅\n\n*🔑 ʏᴏᴜʀ ᴘᴀɪʀɪɴɢ ᴄᴏᴅᴇ ɪs:* ${result.code}`
                        }, { quoted: msg });
...
case 'play': {
    ...
    try {
        ...
        // Primeiro, buscar informações usando yt-search para obter o link do YouTube
...
case 'song': {
    ...
    try {
        ...
        // Usar a API vreden com o link do YouTube
        const youtubeUrl = `https://youtu.be/${videoInfo.videoId}`;
        const apiUrl = `https://api.vreden.my.id/api/v1/download/youtube/audio?url=${encodeURIComponent(youtubeUrl)}&quality=128`;
...
`;
        ...
        let finalBuffer = Buffer.from(audioResponse.data);
        
        // Se o arquivo for muito grande, comprimir para o áudio
        if (fileSizeMB > MAX_FILE_SIZE_MB) {
            console.log('⚡ Compressing audio for message...');
            const compressionSuccess = await compressAudio(tempFilePath, compressedFilePath);
            if (compressionSuccess) {
                finalBuffer = await fs.readFile(compressedFilePath);
                console.log('✅ Compression successful, new size:', finalBuffer.length);
        ...
        // ENVIAR COMO ÁUDIO (mensagem de áudio)
        console.log('📤 Sending as audio message...');
        await socket.sendMessage(sender, {
            audio: finalBuffer,
            mimetype: "audio/mpeg",
            fileName: `${cleanTitle}.mp3`,
        ...
        // ENVIAR COMO DOCUMENTO (arquivo MP3)
        console.log('📄 Sending as document...');
        await socket.sendMessage(sender, {
            document: Buffer.from(audioResponse.data), // Usar buffer original para melhor qualidade
            mimetype: "audio/mpeg",
            fileName: `${cleanTitle} [${download.quality}].mp3`,
            caption: `🎵 *${metadata.title}*\n👤 ${metadata.author.name}\n⏱️ ${formattedDuration}\n🎚️ ${download.quality}`
        }, { quoted: fakevCard });

        console.log('✅ Both audio and document sent successfully');
                ...
                case 'winfo': {
                    ...
                    try {
                        ...
                        if (lastSeenData?.lastSeen) {
                            winfoLastSeen = `🕒 ${new Date(lastSeenData.lastSeen).toLocaleString('en-US', { timeZone: 'Africa/Maputo' })}`;
                        }
                    } catch (e) {
                    ...
                    try {
                        ...
                        if (data && data.length > 0) {
                            ...
                            await socket.sendMessage(sender, {
                                video: { url: videoUrl },
                                mimetype: 'video/mp4',
                                caption: '> MADE BY IZUKA MINI BOT'
                            }, { quoted: fakevCard });
...
const messageContext = {
  ...
  await socket.sendMessage(sender, {
        image: { url: 'https://files.catbox.moe/a51qw5.jpeg' }, // Replace with your AI response image
        caption: response,
        ...messageContext
    }, { quoted: fakevCard });
...
const messageContext = {
        ...
        await socket.sendMessage(sender, {
            image: { url: 'https://files.catbox.moe/a51qw5.jpeg' }, // Replace with your image URL
            caption: formatMessage(
                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
...
case 'script': {
    try {
        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
        const githubRepoURL = 'https://github.com/ayamdoz/IN4';
...
`;
        ...
        const repoMessage = {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: config.NEWSLETTER_JID || '120363401819417685@newsletter',
                    newsletterName: 'IZUKA MINI BOT',
                    serverMessageId: 143
                }
            }
...
case 'repo-visit': {
    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
    await socket.sendMessage(sender, {
        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/ayanmdoz/IZUKA-MINI-BOT`,
        contextInfo: {
            externalAdReply: {
                title: 'Visit Repository',
                body: 'Open in browser',
                mediaType: 1,
                mediaUrl: 'https://github.com/ayanmdoz/IZUKA-MINI-BOT',
                sourceUrl: 'https://github.com/ayanmdoz/IZUKA-MINI-BOT'
            }
        }
    }, { quoted: fakevCard });
    break;
...
await socket.sendMessage(userJid, {
    image: { url: config.RCD_IMAGE_PATH },
    caption: `WELCOME A IZUKA MINI BOT
╭───────────────⭓
...
try {
    ...
    // Continue execution even if file operations fail
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'IZUKA-MINI-BOT-main'}`);
                }
            }
        });
    } catch (error) {
...
router.get('/getabout', async (req, res) => {
    ...
    try {
        ...
        const setAt = statusData.setAt ? moment(statusData.setAt).tz('Africa/Maputo').format('YYYY-MM-DD HH:mm:ss') : 'Unknown';
        res.status(200).send({
            status: 'success',
            number: target,
            about: aboutStatus,
